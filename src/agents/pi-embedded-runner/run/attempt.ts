  ...
  try {
    ...
    try {
      ...
      const { customTools } = splitSdkTools({
        tools: toolsForSdk.map((tool) => wrapToolWithCallLogging(tool)),
...
/** Recursively compact a JSON Schema for small-context local model providers.
 * - Strips description/title/metadata fields
 * - Converts anyOf:[{const:v},...] enum patterns → {type:"string"} (much more compact)
 * - Recurses into arrays so nested schemas are also compacted */
function stripSchemaDescriptions(schema: unknown): unknown {
  if (schema === null || schema === undefined) {
    return schema;
  }
  if (Array.isArray(schema)) {
    return (schema as unknown[]).map((item) => stripSchemaDescriptions(item));
  }
  ...
  // Convert anyOf:[{const:v1},{const:v2},...] enum pattern to bare string type.
  // This pattern is generated by TypeBox for TS enums and is extremely verbose —
  // each const value wraps in an object. A 35B model can infer valid values from context.
  if (Array.isArray(obj.anyOf)) {
    const allConst = (obj.anyOf as unknown[]).every(
      (s) => s !== null && typeof s === "object" && !Array.isArray(s) && "const" in s,
    );
    if (allConst) {
      return { type: "string" };
    }
